...
class TestProbeVideo:
    ...
    def test_probe_video_ffprobe_failure(self, mock_get_ffmpeg, mock_run):
        ...
        mock_run.side_effect = subprocess.CalledProcessError(
            returncode=1, cmd=["ffprobe"], stderr=b"No such file or directory"
        )
...
class TestFastPathDecision:
    ...
    def test_fast_path_enabled_for_cfr(self):
        ...
        assert (
            should_use_fast_path(
                metadata, normalize_to_contract=False, force_cfr=True, fast_path_enabled=True
            )
            is True
        )
    ...
    def test_fast_path_allowed_for_vfr_when_cfr_not_forced(self):
        ...
        assert (
            should_use_fast_path(
                metadata, normalize_to_contract=False, force_cfr=False, fast_path_enabled=True
            )
            is True
        )
    ...
    def test_fast_path_disabled_when_normalization_forced(self):
        ...
        assert (
            should_use_fast_path(
                metadata, normalize_to_contract=True, force_cfr=True, fast_path_enabled=True
            )
            is False
        )
    ...
    def test_fast_path_disabled_globally(self):
        ...
        assert (
            should_use_fast_path(
                metadata, normalize_to_contract=False, force_cfr=True, fast_path_enabled=False
            )
            is False
        )
...
class TestRenderSegmentWithRunner:
    ...
    def test_render_with_runner_cfr_fast_path(self, mock_runner_cls, mock_probe):
        ...
        config = RenderingConfig(fast_path_enabled=True, normalize_to_contract=False)
        result = render_segment_with_runner(
            "input.mp4", 10.0, 20.0, "output.mp4", rendering_config=config
        )
    ...
    def test_render_with_runner_vfr_reencode(self, mock_runner_cls, mock_probe):
        ...
        mock_runner = MagicMock()
        mock_runner.extract_segment.return_value = FfmpegResult(
            success=True, returncode=0, stdout="", stderr="", duration_s=5.0
        )
        mock_runner_cls.return_value = mock_runner
    ...
    def test_render_with_runner_uses_default_config(self, mock_runner_cls, mock_probe):
        ...
        result = render_segment_with_runner("input.mp4", 10.0, 20.0, "output.mp4")
