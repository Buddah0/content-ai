...
class SourceQuery(BaseModel):
    ...
    tags: List[str] = Field(default_factory=list, description="Tags to match (e.g., 'gameplay', 'action')")
    min_quality: float = Field(default=0.0, ge=0.0, le=1.0, description="Minimum quality score")
    preferred_sources: List[str] = Field(default_factory=list, description="Preferred source IDs/libraries")
